...
import com.jianxin.im.widget.InputAwareLayout;
import com.jianxin.im.widget.KeyboardAwareLinearLayout;
  ...
  //  private Observer<List<GroupInfo>> groupInfosUpdateLiveDataObserver;
    ...
    private Observer<UiMessage> messageLiveDataObserver = new Observer<UiMessage>() {
        ...
        public void onChanged(@Nullable UiMessage uiMessage) {
            ...
            MessageContent content = uiMessage.message.getContent();
            if (isDisplayableMessage(uiMessage)) {
                ...
                if (moveToBottom || uiMessage.message.getSender().equals(ChatManager.getInstance().getUserId())) {
        ...
        //    if (content instanceof TypingMessageContent && uiMessage.message.getDirection() == MessageDirection.Receive) {
       //         updateTypingStatusTitle((TypingMessageContent) content);
//            } else {
                resetConversationTitle();
         //   }

            if (getLifecycle().getCurrentState() == Lifecycle.State.RESUMED && uiMessage.message.getDirection() == MessageDirection.Receive) {
